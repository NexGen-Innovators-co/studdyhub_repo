...
// The Gemini API (via generativelanguage.googleapis.com) has specific MIME type support.
// DOCX and other office formats are not supported on this endpoint.
...
serve(async (req) => {
    ...
    try {
        ...
        if (!SUPPORTED_MIME_TYPES.includes(file_type)) {
            return new Response(
                JSON.stringify({ error: `File type '${file_type}' is not supported. Please use PDF or TXT files.` }),
                {
