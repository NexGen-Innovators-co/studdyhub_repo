...
export const RecordingSidePanel: React.FC<RecordingSidePanelProps> = ({
  ...
  return (
    <>
      {/* Backdrop for mobile */}
      <div
        className={`fixed inset-0 bg-black/20 backdrop-blur-sm z-30 transition-opacity lg:hidden ${isExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'
          }`}
        onClick={onClose}
      />

      {/* Panel */}
      <div className={`fixed inset-y-0 right-0 bg-white dark:bg-gray-900 border-l border-slate-200/50 dark:border-gray-700/50 shadow-2xl flex flex-col z-40 transition-all duration-300 ease-out ${isExpanded
        ? 'w-full max-w-2xl'
