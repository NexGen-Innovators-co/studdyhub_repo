...
export const PodcastPanel = forwardRef<PodcastPanelRef, PodcastPanelProps>(({
  ...
  const [segmentLoadStates, setSegmentLoadStates] = useState<Map<number, SegmentLoadState>>(new Map());
  const [loadingSegmentsBatch, setLoadingSegmentsBatch] = useState(false);
  const [currentBatchIndex, setCurrentBatchIndex] = useState(0);
  const BATCH_SIZE = 5; // Load 5 segments at a time
  const segmentLoadQueueRef = useRef<Set<number>>(new Set());
  const [transcriptLoadStates, setTranscriptLoadStates] = useState<Map<number, boolean>>(new Map());
  const transcriptObserverRef = useRef<IntersectionObserver | null>(null);

  // Add type definition at top of file (after line 36)
  // interface SegmentLoadState {
  //   index: number;
  //   loaded: boolean;
  //   loading: boolean;
 ...
 // Progressive Loading Function
    const loadSegmentBatch = useCallback(async (startIndex: number, batchSize: number) => {
      if (!podcast?.audioSegments) return;
      const endIndex = Math.min(startIndex + batchSize, podcast.audioSegments.length);
      const segmentsToLoad = podcast.audioSegments.slice(startIndex, endIndex);
      setLoadingSegmentsBatch(true);
      const newStates = new Map(segmentLoadStates);
      ...
      setSegmentLoadStates(newStates);
      // Simulate progressive loading (300-500ms delay for smooth UX)
      await new Promise(resolve => setTimeout(resolve, 300 + Math.random() * 200));
      const updatedStates = new Map(newStates);
      for (let i = startIndex; i < endIndex; i++) {
        updatedStates.set(i, {
          index: i,
          loaded: true,
          loading: false,
          error: false,
          data: podcast.audioSegments[i]
        });
      }
      setSegmentLoadStates(updatedStates);
      setLoadingSegmentsBatch(false);
      setCurrentBatchIndex(endIndex);
    }, [podcast?.audioSegments, segmentLoadStates]);
...
// Lazy loading for transcripts
  useEffect(() => {
    if (!showTranscript || !isOpen) return;
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const segmentIndex = parseInt(entry.target.getAttribute('data-segment-index') || '0');
            setTranscriptLoadStates(prev => {
              const next = new Map(prev);
              next.set(segmentIndex, true);
              return next;
            });
          }
        });
      ...
      {
        root: transcriptContainerRef.current,
        rootMargin: '100px',
        threshold: 0.1
      }
    );
    transcriptObserverRef.current = observer;
    // Observe all transcript segments
  ...
  }, [showTranscript, isOpen, podcast?.audioSegments]);
  ...
  // Initialize progressive loading on podcast open
  useEffect(() => {
    if (!isOpen || !podcast) return;
    setSegmentLoadStates(new Map());
    setCurrentBatchIndex(0);
    setLoadingSegmentsBatch(false);
    // Start loading first batch
    if (podcast.audioSegments && podcast.audioSegments.length > 0) {
      loadSegmentBatch(0, BATCH_SIZE);
    }
  }, [isOpen, podcast?.id]);

  // Auto-load next batch when user approaches end of current batch
  useEffect(() => {
    if (!podcast?.audioSegments || loadingSegmentsBatch) return;
    const totalSegments = podcast.audioSegments.length;
    const needsMoreSegments = currentSegmentIndex >= currentBatchIndex - 2 && currentBatchIndex < totalSegments;
    if (needsMoreSegments) {
      loadSegmentBatch(currentBatchIndex, BATCH_SIZE);
    }
  }, [currentSegmentIndex, currentBatchIndex, loadingSegmentsBatch, podcast?.audioSegments, loadSegmentBatch]);
