...
export const QuizHistory: React.FC<QuizHistoryProps> = ({ quizzes, onSelectQuiz, compact = false, bestAttempts = {} }) => {
  const inferSourceType = (quiz: Quiz) => {
    if (quiz.source_type) return quiz.source_type;
    if (quiz.classId === 'ai-generated') return 'ai';
    if (quiz.classId === 'notes-generated') return 'notes';
    return 'recording';
  };
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {bestAttempts[quiz.id] && (
                        <Badge className="bg-green-100 text-green-700 border-green-200">
                          Best Score: {bestAttempts[quiz.id].percentage}%
                        </Badge>
                      )}
                    </div>
